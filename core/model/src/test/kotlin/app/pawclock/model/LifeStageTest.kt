...
class LifeStageTest {
    ...
    @Test
    fun `Bird life stages all exist`() {
        val stages = LifeStage.Bird.all()
        assertEquals(5, stages.size)
        assertTrue(stages.contains(LifeStage.Bird.Hatchling))
        assertTrue(stages.contains(LifeStage.Bird.Juvenile))
        assertTrue(stages.contains(LifeStage.Bird.Adult))
        assertTrue(stages.contains(LifeStage.Bird.Senior))
        assertTrue(stages.contains(LifeStage.Bird.Geriatric))
    }

    @Test
    fun `Bird stages have stable displayKeys`() {
        assertEquals("bird_hatchling", LifeStage.Bird.Hatchling.displayKey)
        assertEquals("bird_juvenile", LifeStage.Bird.Juvenile.displayKey)
        assertEquals("bird_adult", LifeStage.Bird.Adult.displayKey)
        assertEquals("bird_senior", LifeStage.Bird.Senior.displayKey)
        assertEquals("bird_geriatric", LifeStage.Bird.Geriatric.displayKey)
    }

    @Test
    fun `Bird stages have monotonic ordinals from Hatchling to Geriatric`() {
        val expectedOrder =
            listOf(
                LifeStage.Bird.Hatchling,
                LifeStage.Bird.Juvenile,
                LifeStage.Bird.Adult,
                LifeStage.Bird.Senior,
                LifeStage.Bird.Geriatric,
            )
        expectedOrder.zipWithNext().forEach { (a, b) ->
            assertTrue(a.ordinal < b.ordinal, "${a.displayKey}.ordinal should be < ${b.displayKey}.ordinal")
        }
    ...
    @Test
    fun `Reptile life stages all exist`() {
        val stages = LifeStage.Reptile.all()
        assertEquals(4, stages.size)
        assertTrue(stages.contains(LifeStage.Reptile.Hatchling))
        assertTrue(stages.contains(LifeStage.Reptile.Juvenile))
        assertTrue(stages.contains(LifeStage.Reptile.Adult))
        assertTrue(stages.contains(LifeStage.Reptile.Senior))
    }

    @Test
    fun `Reptile stages have stable displayKeys`() {
        assertEquals("reptile_hatchling", LifeStage.Reptile.Hatchling.displayKey)
        assertEquals("reptile_juvenile", LifeStage.Reptile.Juvenile.displayKey)
        assertEquals("reptile_adult", LifeStage.Reptile.Adult.displayKey)
        assertEquals("reptile_senior", LifeStage.Reptile.Senior.displayKey)
    }

    @Test
    fun `Reptile stages have monotonic ordinals from Hatchling to Senior`() {
        val expectedOrder =
            listOf(
                LifeStage.Reptile.Hatchling,
                LifeStage.Reptile.Juvenile,
                LifeStage.Reptile.Adult,
                LifeStage.Reptile.Senior,
            )
        expectedOrder.zipWithNext().forEach { (a, b) ->
            assertTrue(a.ordinal < b.ordinal, "${a.displayKey}.ordinal should be < ${b.displayKey}.ordinal")
        }
    ...
    @Test
    fun `Horse life stages all exist`() {
        val stages = LifeStage.Horse.all()
        assertEquals(5, stages.size)
        assertTrue(stages.contains(LifeStage.Horse.Foal))
        assertTrue(stages.contains(LifeStage.Horse.Yearling))
        assertTrue(stages.contains(LifeStage.Horse.YoungAdult))
        assertTrue(stages.contains(LifeStage.Horse.Adult))
        assertTrue(stages.contains(LifeStage.Horse.Senior))
    }

    @Test
    fun `Horse stages have stable displayKeys`() {
        assertEquals("horse_foal", LifeStage.Horse.Foal.displayKey)
        assertEquals("horse_yearling", LifeStage.Horse.Yearling.displayKey)
        assertEquals("horse_young_adult", LifeStage.Horse.YoungAdult.displayKey)
        assertEquals("horse_adult", LifeStage.Horse.Adult.displayKey)
        assertEquals("horse_senior", LifeStage.Horse.Senior.displayKey)
    }

    @Test
    fun `Horse stages have monotonic ordinals from Foal to Senior`() {
        val expectedOrder =
            listOf(
                LifeStage.Horse.Foal,
                LifeStage.Horse.Yearling,
                LifeStage.Horse.YoungAdult,
                LifeStage.Horse.Adult,
                LifeStage.Horse.Senior,
            )
        expectedOrder.zipWithNext().forEach { (a, b) ->
            assertTrue(a.ordinal < b.ordinal, "${a.displayKey}.ordinal should be < ${b.displayKey}.ordinal")
        }
    ...
    @Test
    fun `Fish life stages all exist`() {
        val stages = LifeStage.Fish.all()
        assertEquals(4, stages.size)
        assertTrue(stages.contains(LifeStage.Fish.Fry))
        assertTrue(stages.contains(LifeStage.Fish.Juvenile))
        assertTrue(stages.contains(LifeStage.Fish.Adult))
        assertTrue(stages.contains(LifeStage.Fish.Senior))
    }

    @Test
    fun `Fish stages have stable displayKeys`() {
        assertEquals("fish_fry", LifeStage.Fish.Fry.displayKey)
        assertEquals("fish_juvenile", LifeStage.Fish.Juvenile.displayKey)
        assertEquals("fish_adult", LifeStage.Fish.Adult.displayKey)
        assertEquals("fish_senior", LifeStage.Fish.Senior.displayKey)
    }

    @Test
    fun `Fish stages have monotonic ordinals from Fry to Senior`() {
        val expectedOrder =
            listOf(
                LifeStage.Fish.Fry,
                LifeStage.Fish.Juvenile,
                LifeStage.Fish.Adult,
                LifeStage.Fish.Senior,
            )
        expectedOrder.zipWithNext().forEach { (a, b) ->
            assertTrue(a.ordinal < b.ordinal, "${a.displayKey}.ordinal should be < ${b.displayKey}.ordinal")
        }
