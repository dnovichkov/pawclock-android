...
/**
 * Селектор подкатегории (размер/тип в зависимости от вида) для Quick Calculator
 * (Task 20 / Plan 1; расширен на все 12 видов в Task 16 / Plan 2).
 *
 ...
 *
 * Метка резолвится через `(species, id)`, а не только по `id`: id подкатегорий пересекаются
 * между видами (например, `small`/`medium`/`large`/`giant` есть и у [app.pawclock.model.DogSize],
 * и у [app.pawclock.model.RabbitSize]; `dwarf` — у Rabbit и Hamster), поэтому без species
 * корректную метку выбрать нельзя.
 */
...
/**
 * Маппит подкатегорию `(species, id)` на string-resource id. Возвращает `0` для неизвестных
 * комбинаций, чтобы caller упал на `option.label` без NPE.
 *
 * Диспетчеризация по виду + per-species под-функции: id пересекаются между видами,
 * единый плоский `when (id)` дал бы коллизии и высокую цикломатическую сложность.
 */
